...
import SuggestedStory from './components/SuggestedStory'
const windowWidth = Dimensions.get('window').width;

export default function App() {
  const names = ["Afaf", "Samyah", "Sami", "Mazen", "Murooj", "Khaled", "Nawal", "Yazen", "Yazeed"]
    ...
    <View style={styles.container}>
      ...
      <View>
        <ScrollView style={styles.scrollV} horizontal showsHorizontalScrollIndicator={false}>
          {result.map((name, index) => (
            <View key={index} style={styles.stories}>
              <SuggestedStory key={index} username={name} />
            </View>
          ))}
        </ScrollView>
      </View>
